//
//  UIViewController+URURLRouter.m
//  PassNurseExam
//
...
#import "UIViewController+URURLRouter.h"
...
@implementation UIViewController (URURLRouter)
...
+ (UIViewController *)initFromURL:(NSURL *)url withQuery:(NSDictionary *)query fromConfig:(NSDictionary *)configDict{
    ...
    if([configDict.allKeys containsObject:url.scheme]){ // 字典中的所有的key是否包含传入的协议头
        ...
        if([config isKindOfClass:[NSString class]]){ //当协议头是http https的情况
            newClass =  NSClassFromString(config);
            ...
            if([dict.allKeys containsObject:home]){
                newClass =  NSClassFromString([dict objectForKey:home]); // 根据key拿到对应的控制器名称
                if (newClass == nil) { // 兼容swift,字符串转类名的时候前面加上命名空间
                    NSString *spaceName = [NSBundle mainBundle].infoDictionary[@"CFBundleExecutable"];
                    newClass =  NSClassFromString([NSString stringWithFormat:@"%@.%@",spaceName,[dict objectForKey:home]]);
                }
            }
        }
        if(newClass !=nil){
        ...
        if ([url.scheme isEqualToString:@"http"] || [url.scheme isEqualToString:@"https"]) {
            newClass =  NSClassFromString([configDict objectForKey:url.scheme]);
