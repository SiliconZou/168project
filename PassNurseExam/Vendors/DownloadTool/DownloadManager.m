...
- (void)download:(NSString *)urlString model:(CourseCommonDetailDataCurriculumsModel *)model progress:(void (^)(TaskProgressInfo *))progressInfo complete:(void (^)(NSString *, NSError *))complete enableBackgroundMode:(BOOL)backgroundMode{
...
- (void)cancleDownload:(NSString *)urlString
{
    ...
    CourseCommonDetailDataCurriculumsModel *model = [self modelHasAddToModelArray:urlString];
    if (model) {
...
- (CourseCommonDetailDataCurriculumsModel *)modelHasAddToModelArray:(NSString *)url{
    CourseCommonDetailDataCurriculumsModel *model = nil;
    for (CourseCommonDetailDataCurriculumsModel *m in self.modelArray) {
        if ([m.download_address isEqualToString:url]) {
            model = m;
        }
    }
    return model;
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data{
    [self.downloadArray enumerateObjectsUsingBlock:^(DownloadObject *downloadObj,NSUInteger index, BOOL *stop){
        if ([downloadObj.downloadTask isEqual:dataTask]) {
            ...
            NSUInteger nwr = 0;
            do {
                nwr = [downloadObj.stream write:(const unsigned char *)[data bytes] maxLength:left];
                if (-1 == nwr) {
                    break;
                }
                left -= nwr;
            ...
            if (downloadObj.progressBlock) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (downloadObj.progressInfo.progress == 1) {
                        ...
                        CourseCommonDetailDataCurriculumsModel *model = [self modelHasAddToModelArray:downloadObj.progressInfo.url];
