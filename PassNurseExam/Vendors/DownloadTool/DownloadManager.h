 ...
 *  @param progressInfo       文件下载过程的回调block
 *  @param complete       文件下载完后的回调block
 *  @param backgroundMode 后台模式选择
 */
- (void)download:(NSString *)urlString model:(CourseCommonDetailDataCurriculumsModel *)model progress:(void(^)(TaskProgressInfo *progressInfo))progressInfo complete:(void(^)(NSString *filePath,NSError *error))complete enableBackgroundMode:(BOOL)backgroundMode;
/**
 *  暂停正在进行的下载任务
 */
