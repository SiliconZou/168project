...
#import "URToastHelper.h"

@interface URToastHelper ()

@property (nonatomic, readwrite) URToastMaskType maskType;
...
- (void)showWithStatus:(NSString*)string maskType:(URToastMaskType)hudMaskType networkIndicator:(BOOL)show;
...
+ (URToastHelper *)sharedView
{
    static dispatch_once_t once;
    static URToastHelper *sharedView;
    dispatch_once(&once, ^ { sharedView = [[URToastHelper alloc] initWithFrame:[[UIScreen mainScreen] bounds]]; });
    return sharedView;
}
...
+ (void)setStatus:(NSString *)string
{
    [[URToastHelper sharedView] setStatus:string];
}
...
+ (void)show
{
    [[URToastHelper sharedView] showWithStatus:nil maskType:URToastMaskTypeNone networkIndicator:NO];
}

+ (void)showWithStatus:(NSString *)status
{
    [[URToastHelper sharedView] showWithStatus:status maskType:URToastMaskTypeNone networkIndicator:NO];
}

+ (void)showWithMaskType:(URToastMaskType)maskType
{
    [[URToastHelper sharedView] showWithStatus:nil maskType:maskType networkIndicator:NO];
}

+ (void)showWithStatus:(NSString*)status maskType:(URToastMaskType)maskType
{
    [[URToastHelper sharedView] showWithStatus:status maskType:maskType networkIndicator:NO];
}

+ (void)showErrorWithStatus:(NSString *)string
{
    [URToastHelper showErrorWithStatus:string duration:1.8f];
}

+ (void)showErrorWithStatus:(NSString *)string duration:(NSTimeInterval)duration
{
    [URToastHelper show];
    [URToastHelper dismissWithError:string afterDelay:duration];
}
...
+ (void)dismiss
{
    [[URToastHelper sharedView] dismiss];
}

+ (void)dismissWithSuccess:(NSString*)successString
{
    [[URToastHelper sharedView] dismissWithStatus:successString error:NO];
}

+ (void)dismissWithSuccess:(NSString *)successString afterDelay:(NSTimeInterval)seconds
{
    [[URToastHelper sharedView] dismissWithStatus:successString error:NO afterDelay:seconds];
}

+ (void)dismissWithError:(NSString*)errorString
{
    [[URToastHelper sharedView] dismissWithStatus:errorString error:YES];
}

+ (void)dismissWithError:(NSString *)errorString afterDelay:(NSTimeInterval)seconds
{
    [[URToastHelper sharedView] dismissWithStatus:errorString error:YES afterDelay:seconds];
}
...
+ (void)dismissWithError:(NSString *)successString title:(NSString *)titleString andImage:(UIImage *)image afterDelay:(NSTimeInterval)duration {
    [[URToastHelper sharedView] dismissWithStatus:successString title:titleString Image:image afterDelay:duration];
}
...
- (void)showWithStatus:(NSString*)string maskType:(URToastMaskType)hudMaskType networkIndicator:(BOOL)show
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(self.maskType != URToastMaskTypeNone)
        {
...
+ (BOOL)isVisible
{
    return ([URToastHelper sharedView].alpha == 1);
}
